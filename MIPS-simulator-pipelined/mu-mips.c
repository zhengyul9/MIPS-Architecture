...
void EX()
{
	 //copied over from lab 1 print instruction to save time
        EX_MEM.IR = ID_EX.IR;
	EX_MEM.PC = ID_EX.PC;
	...
	instruction = ID_EX.IR; 
        ...
        uint32_t bk = instruction & 0x0000003F; //back of 32 bits to id the op code
      //  uint8_t rs = (instruction & 0x03E00000) >> 21; //source
        uint64_t rt = (instruction & 0x001F0000) >> 16; //destination
       // uint32_t im = instruction & 0x0000FFFF; //immediate / offset change to 32bits for the sign-extend
       // uint8_t rd = (instruction & 0x0000F800) >> 11; //reg destination (register)
      //  uint8_t h = (instruction & 0x0000007C0) >> 6; //for shift instructions) **** know as SA in the BOOK
...
void ID()
{
	...
	//some was copied over from lab 1 print instruction to save time
	ID_EX.IR = IF_ID.IR;
	...
	uint32_t op = (instruction & 0xFC000000) >>26; //opcode
	uint32_t bk = instruction & 0x0000003F; //back of 32 bits to id the op code
	uint32_t rs = (instruction & 0x03E00000) >> 21; //source
	uint32_t rt = (instruction & 0x001F0000) >> 16; //destination
	uint32_t im = instruction & 0x0000FFFF; //immediate / offset change to 32bits for the sign-extend
//	uint8_t rd = (instruction & 0x0000F800) >> 11; //reg destination (register)
	...
	if(op == 0X00)
	{
		switch(bk)
		{
			case 0x00://SLL last of the bk should be 000000
