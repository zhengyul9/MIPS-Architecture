		...
		case 0x00000000: //ADD, ADDU, AND, NOR, OR, SLT, SUB, SUBU, XOR, SLL, SRL, DIV, DIVU, MULT, MULTU, SRA, MFHI, MFLO, MTHI, MTLO, JR, JALR
			switch(bk) {
				...
				case 0x00000011: //MTHI
					sprintf(string, "MTHI $%d\n", rs);
					break;
				case 0x00000013: //MTLO
					sprintf(string, "MTLO $%d\n", rs);
					break;
				case 0x00000008: //JR
					sprintf(string, "JR $%d\n", rs);
					break;
				case 0x00000009: //JALR
					sprintf(string, "JALR $%d\n", rs);
					break;
...
void MEM() {
		...
		switch(opcode){
        	...
        	case 0x21: //LH
            	data = mem_read_32(EX_MEM.ALUOutput);
            	MEM_WB.LMD = (data & 0x8000) > 0 ? (data | 0xFFFF0000) : ( data & 0x0000FFFF);
            	break;
        	case 0x23: //LW
            	data = mem_read_32(EX_MEM.ALUOutput);
            	MEM_WB.LMD = data;
...
void ID() {
	...
	if(op == 0X00) {
		switch(bk) {
			case 0x00://SLL last of the bk shouldSIM be 000000
				ID_EX.A = CURRENT_STATE.REGS[ID_EX_rt];
				ID_EX.B = h;
				ID_EX.imm = 0;
				break;
	...
	int EX_OP = ((EX_MEM.IR & 0xFC000000) >> 26);
	if( EX_OP == 0x23 || EX_OP == 0x21 || EX_OP == 0x20) { //if load, stall once always
		if(stall == 0) {
			stall = 2;
		}
	}
